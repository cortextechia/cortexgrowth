  ...
  return (
    <ProtectedRoute>
      <div className="min-h-screen flex">
        {/* Sidebar */}
        <aside
          className={`${sidebarOpen ? 'w-56' : 'w-16'} fixed h-screen flex flex-col transition-all duration-200 z-20`}
          style={{ background: '#05091200', borderRight: '1px solid rgba(255,255,255,0.06)' }}
        >
          <div style={{ backgroundColor: '#060c1a' }} className="flex flex-col h-full">
            {/* Logo + toggle */}
            <div
              className={`flex items-center h-14 ${sidebarOpen ? 'justify-between px-4' : 'justify-center'}`}
              style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}
            >
              {sidebarOpen && (
                <div className="flex items-center gap-2">
                  <div className="h-6 w-6 rounded-md flex items-center justify-center" style={{ backgroundColor: '#3b82f6' }}>
              ...
              {NAV_ITEMS.map((item) => {
                ...
                const link = (
                  <Link
                    key={item.href}
                    href={item.href}
                    className={`flex items-center gap-3 rounded-lg px-2.5 py-2 text-sm transition-colors ${!sidebarOpen ? 'justify-center' : ''}`}
                    style={
                      isActive
                        ? { backgroundColor: 'rgba(59,130,246,0.12)', color: '#60a5fa', borderLeft: '2px solid #3b82f6', paddingLeft: sidebarOpen ? '8px' : undefined }
                        : { color: '#64748b' }
                    }
        ...
        <main className={`flex-1 ${sidebarOpen ? 'ml-56' : 'ml-16'} transition-all duration-200 min-h-screen`}>
          ...
          >
            <div className="px-6 h-14 flex items-center justify-between">
              <span className="text-sm font-medium" style={{ color: '#f1f5f9' }}>{organization?.name ?? 'Dashboard'}</span>
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm" style={{ color: '#64748b' }}>{user?.name}</span>
              </div>
            </div>
          </div>
