          ...
          <div key={step.id} className="flex items-center flex-1 last:flex-none">
            <div
              ...
              style={
                i < currentIndex
                  ...
                  : i === currentIndex
                  ? { backgroundColor: 'rgba(59,130,246,0.15)', border: '2px solid #3b82f6', color: '#60a5fa' }
                  : { backgroundColor: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.3)' }
              }
            >
            ...
            {i < STEPS.length - 1 && (
              <div
                ...
                style={{ backgroundColor: i < currentIndex ? '#3b82f6' : 'rgba(255,255,255,0.08)' }}
              />
            )}
          </div>
        ))}
      ...
      <div className="flex justify-between">
        {STEPS.map((step, i) => (
          <span
            key={step.id}
            className="text-xs"
            style={{ color: i === currentIndex ? '#94a3b8' : 'rgba(255,255,255,0.2)', minWidth: 0 }}
          >
            {step.label}
          </span>
        ))}
  ...
  return (
    <div className="text-center">
      <div className="mb-6 flex items-center justify-center">
        ...
        >
          <svg className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5} style={{ color: '#60a5fa' }}>
      ...
      <div className="space-y-3 text-left mb-8 rounded-xl p-4" style={{ backgroundColor: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)' }}>
        {[
          ...
          <div key={i} className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-medium" style={{ color: '#cbd5e1' }}>{item.label}</p>
  ...
  return (
    <div className="text-center">
      ...
      {isLoading ? (
        ...
        >
          <div className="flex items-center justify-center h-8 w-8 rounded-full" style={{ backgroundColor: 'rgba(34,197,94,0.15)' }}>
            <CheckIcon className="h-4 w-4" />
          </div>
          <div className="text-left">
            <p className="text-sm font-medium" style={{ color: '#4ade80' }}>{cfg.label} conectado</p>
            <p className="text-xs" style={{ color: '#475569' }}>Integração ativa e sincronizando dados</p>
          </div>
        </div>
      ) : (
        <button
      ...
      <div className="flex flex-col gap-3 mt-4">
        <button
          ...
          style={{ backgroundColor: isConnected ? '#3b82f6' : 'rgba(59,130,246,0.3)', color: isConnected ? '#fff' : '#60a5fa' }}
          onMouseEnter={e => { if (isConnected) e.currentTarget.style.backgroundColor = '#2563eb'; }}
          onMouseLeave={e => { if (isConnected) e.currentTarget.style.backgroundColor = '#3b82f6'; }}
        >
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-center">
        <div
          className="h-16 w-16 rounded-2xl flex items-center justify-center"
          style={{ backgroundColor: 'rgba(168,85,247,0.12)', border: '1px solid rgba(168,85,247,0.3)' }}
        >
          <svg className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5} style={{ color: '#c084fc' }}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M12 21a9.004 9.004 0 008.716-6.747M12 21a9.004 9.004 0 01-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3m0 0a8.997 8.997 0 017.843 4.582M12 3a8.997 8.997 0 00-7.843 4.582m15.686 0A11.953 11.953 0 0112 10.5c-2.998 0-5.74-1.1-7.843-2.918m15.686 0A8.959 8.959 0 0121 12c0 .778-.099 1.533-.284 2.253m0 0A17.919 17.919 0 0112 16.5c-3.162 0-6.133-.815-8.716-2.247m0 0A9.015 9.015 0 013 12c0-1.605.42-3.113 1.157-4.418" />
          </svg>
        </div>
      </div>

      <h1 className="text-xl font-bold mb-2 text-center" style={{ color: '#f1f5f9' }}>
        URL do seu site
      </h1>
      ...
      {error && (
        <div className="mb-4 rounded-lg px-3 py-2.5 text-xs" style={{ backgroundColor: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.2)', color: '#f87171' }}>
          {error}
        </div>
      )}
      ...
      <div className="space-y-4">
        <div>
          <label className="block text-xs font-medium mb-1.5" style={{ color: '#94a3b8' }}>
            URL do seu site <span style={{ color: '#475569' }}>(obrigatório para diagnóstico)</span>
          </label>
          <input
            type="url"
            value={websiteUrl}
        ...
        <div>
          <label className="block text-xs font-medium mb-1.5" style={{ color: '#94a3b8' }}>
            Concorrentes <span style={{ color: '#475569' }}>(opcional, até 3)</span>
          </label>
      ...
      <div className="flex flex-col gap-3 mt-6">
        <button
          ...
          style={{ backgroundColor: saved ? '#16a34a' : '#3b82f6' }}
          onMouseEnter={e => { if (!isSaving && !saved) e.currentTarget.style.backgroundColor = '#2563eb'; }}
          onMouseLeave={e => { if (!isSaving && !saved) e.currentTarget.style.backgroundColor = '#3b82f6'; }}
        >
          {isSaving && <Spinner />}
  ...
  return (
    <div className="text-center">
      <div className="mb-6 flex items-center justify-center">
        ...
        >
          <svg className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} style={{ color: '#4ade80' }}>
      ...
      >
        <p className="text-xs font-medium" style={{ color: '#64748b' }}>Próximos passos sugeridos:</p>
        {[
          ...
          <div key={i} className="flex items-center gap-2 text-xs" style={{ color: '#475569' }}>
            <svg className="h-3.5 w-3.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} style={{ color: '#3b82f6' }}>
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-2 rounded-lg py-2.5 text-sm font-medium text-white transition-all disabled:opacity-60"
        style={{ backgroundColor: '#3b82f6' }}
        onMouseEnter={e => { if (!isFinishing) e.currentTarget.style.backgroundColor = '#2563eb'; }}
        onMouseLeave={e => { if (!isFinishing) e.currentTarget.style.backgroundColor = '#3b82f6'; }}
      >
        {isFinishing && <Spinner />}
...
function OnboardingContent() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4" style={{ backgroundColor: '#080d19' }}>
      {/* Background gradient */}
